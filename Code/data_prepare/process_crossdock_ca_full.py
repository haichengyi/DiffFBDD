...
def process_ligand_and_pocket(pdbfile, sdffile,
    ...
    if ca_full:
        try:
            ...
            for res in pocket_residues:
                for atom in res.get_atoms():
                    if atom.name == 'CA':
                        pocket_one_hot_ca.append(np.eye(1, len(amino_acid_dict),
                                                     amino_acid_dict[three_to_one(res.get_resname())]).squeeze())
                        ca_coords.append(atom.coord)
                    else:
                        pocket_one_hot_full.append(np.eye(1, len(amino_acid_dict),
                                                     amino_acid_dict[three_to_one(res.get_resname())]).squeeze())
                        full_coords.append(atom.coord)
            pocket_one_hot_ca = np.stack(pocket_one_hot_ca)
            pocket_one_hot_full = np.stack(pocket_one_hot_full)
        ...
        pocket_data = {
            'pocket_ca': ca_coords,
            'pocket_full':full_coords,
            'pocket_one_hot_ca': pocket_one_hot_ca,
            'pocket_one_hot_full':pocket_one_hot_full,
...
def saveall(filename, pdb_and_mol_ids, lig_coords, lig_one_hot, lig_mask,
            pocket_c_alpha_ca, pocket_one_hot_ca, pocket_mask_ca,
            pocket_c_alpha_full,pocket_one_hot_full,pocket_mask_full):
    np.savez(filename,
             names=pdb_and_mol_ids,
             lig_coords=lig_coords,
             lig_one_hot=lig_one_hot,
             lig_mask=lig_mask,
             pocket_c_alpha_ca=pocket_c_alpha_ca,
             pocket_one_hot_ca=pocket_one_hot_ca,
             pocket_mask_ca=pocket_mask_ca,
             pocket_c_alpha_full = pocket_c_alpha_full,
             pocket_c_one_hot_full = pocket_one_hot_full,
             pocket_mask_full = pocket_mask_full
             )
    return True
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ca_full', action='store_true')  # 同时保存ca和full原子表示法
    ...
    if args.outdir is None:
        ...
        suffix += '_ca_full_temp' if args.ca_full else '_full_temp'
    ...
    #data_split['train'] = random.sample(data_split['train'],500)
    ...
    for split in data_split.keys():
        ...
        for pocket_fn, ligand_fn in pbar:
            ...
            try:
                if args.ca_full:
                    ligand_data, pocket_data = process_ligand_and_pocket(
                        pdbfile, sdffile,
                        atom_dict=atom_dict, dist_cutoff=args.dist_cutoff,ca_full=args.ca_full)
            ...
            pocket_c_alpha_ca.append(pocket_data['pocket_ca'])
            pocket_one_hot_ca.append(pocket_data['pocket_one_hot_ca'])
            pocket_mask_ca.append(count * np.ones(len(pocket_data['pocket_ca'])))
            pocket_c_alpha_full.append(pocket_data['pocket_full'])
            pocket_one_hot_full.append(pocket_data['pocket_one_hot_full'])
            pocket_mask_full.append(count * np.ones(len(pocket_data['pocket_full'])))
            count_protein.append(pocket_data['pocket_ca'].shape[0])
            coLunt_ligand.append(ligand_data['lig_coords'].shape[0])
        ...
        pocket_c_alpha_ca = np.concatenate(pocket_c_alpha_ca, axis=0)
        pocket_one_hot_ca = np.concatenate(pocket_one_hot_ca, axis=0)
        pocket_mask_ca = np.concatenate(pocket_mask_ca, axis=0)
        pocket_c_alpha_full = np.concatenate(pocket_c_alpha_full, axis=0)
        pocket_one_hot_full = np.concatenate(pocket_one_hot_full, axis=0)
        pocket_mask_full = np.concatenate(pocket_mask_full, axis=0)
        saveall(processed_dir / f'{split}.npz', pdb_and_mol_ids, lig_coords,
                lig_one_hot, lig_mask,
                pocket_c_alpha_ca,pocket_one_hot_ca, pocket_mask_ca,
                pocket_c_alpha_full,pocket_one_hot_full, pocket_mask_full)
    ...
    with np.load(processed_dir / 'train.npz', allow_pickle=True) as data:
        ...
        pocket_mask = data['pocket_mask_ca']
        lig_coords = data['lig_coords']
        lig_one_hot = data['lig_one_hot']
        pocket_one_hot = data['pocket_one_hot_ca']
