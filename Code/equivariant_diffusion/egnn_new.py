...
class EGNN(nn.Module):
    ...
    def forward(self, h, x, edge_index,h_pocket,x_pocket,edge_pocket,mask_residues,node_mask=None, edge_mask=None, update_coords_mask=None):
        # Edit Emiel: Remove velocity as input
        # print(h,x,h_pocket)
        distances_pocket, _ = coord2diff(x_pocket, edge_pocket)
        if self.sin_embedding is not None:
            distances = self.sin_embedding(distances_pocket)
        h_pocket = self.embedding(h_pocket)
        for i in range(0, self.n_layers):
            h_pocket, x_pocket = self._modules["e_block_%d" % i](
                h_pocket, x_pocket, edge_pocket, node_mask=node_mask, edge_mask=edge_mask,
                edge_attr=distances_pocket, update_coords_mask=None)
        # Important, the bias of the last linear might be non-zero
        h_pocket = self.embedding_out(h_pocket)
        # print(h_pocket,x_pocket)
       #change the x and h
        len_atom = len(x)-len(mask_residues) 
        x = torch.cat([x[:len_atom],x_pocket[:len(mask_residues)]],dim=0)
        h = torch.cat([h[:len_atom],h_pocket[:len(mask_residues)]],dim=0)
       #print(h_pocket,x_pocket,len_ca,edges_pocket)
        # Edit Emiel: Remove velocity as input
